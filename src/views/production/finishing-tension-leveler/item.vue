  ...
  export default {
    ...
    methods: {
      ...
      handleProductNumberChange(val) {
        if (val) {
          getOutboundOrderRawItems({ next_operation_label: '精整拉矫', product_number: val }).then(res => {
      ...
      handleMaterialNumberChange(val) {
        if (val) {
          getOutboundOrderRawItems({ next_operation_label: '精整拉矫', material_number: val }).then(res => {
            if (res.queryResult.list.length == 1) {
