    ...
    methods: {
      ...
      getPendingRawItemsByNumberType(type) {
        return query => {
          if (query !== '') {
            this.loading = true
            getOutboundOrderRawItems({ next_operation_label: '轧机' }).then(res => {
              this.loading = false
      ...
      handleProductNumberChange(val) {
        if (val) {
          getOutboundOrderRawItems({ next_operation_label: '轧机', product_number: val }).then(res => {
      ...
      handleMaterialNumberChange(val) {
        if (val) {
          getOutboundOrderRawItems({ next_operation_label: '轧机', material_number: val }).then(res => {
            if (res.queryResult.list.length == 1) {
