  ...
  <div class="inbound-order-detail">
    <div v-show="!dialogFormVisible">
      ...
      <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row>
        ...
        <el-table-column label="已收数量" min-width="80px" align="center">
          <template slot-scope="scope">
    ...
    <div v-show="dialogFormVisible">
      <div style="font-size: 20px;">{{textMap[dialogStatus]}}原料入库明细</div>
...
<script>
  ...
  import {
    getInboundOrderRawDetails,
    addInboundOrderRawDetail,
    updateInboundOrderRawDetail,
    deleteInboundOrderRawDetail
  } from '@/api/inboundorderrawdetail.js'
  import { getMaterials } from '@/api/material'

  import waves from '@/directive/waves' // Waves directive
  import Pagination from '@/components/Pagination/index.vue' // Secondary package based on el-pagination
  import Material from './material.vue'
  import InboundOrderRawItem from './InboundOrderRawItem.vue'
  import { getInboundOrderRawItems } from '@/api/inboundorderrawitem'

  export default {
    name: 'InboundOrderRawDetail',
    components: { Pagination, Material, InboundOrderRawItem },
    directives: { waves },
    props: ['orderId'],
    data() {
      return {
        ...
        listQuery: {
          current: 1,
          size: 10,
          inboundOrderRawId: this.orderId
        },
    ...
    //     this.listQuery.inboundOrderRawId = val
    //     this.temp.inboundOrderRawId = val
    //     this.handleFilter()
    //   }
    // },
    watch: {
      'temp.receivedQuantity': {
    ...
    methods: {
      getList() {
        ...
        getInboundOrderRawDetails(this.listQuery).then(res => {
      ...
      handleAdd() {
        this.resetForm('inboundOrderRawDetailForm')
        this.dialogStatus = 'create'
        this.dialogFormVisible = true
        // this.rules.password[0].required = true
        this.$nextTick(() => {
          this.$refs['inboundOrderRawDetailForm'].clearValidate()
        })
      },
      submit() {
        this.$refs['inboundOrderRawDetailForm'].validate((valid) => {
          if (valid) {
            // const tempData = deepClone(this.temp)
            let inboundOrderRawDetail = deepClone(this.temp)
            delete inboundOrderRawDetail.materialCode
            delete inboundOrderRawDetail.materialName
            addInboundOrderRawDetail(inboundOrderRawDetail).then((res) => {
      ...
      updateData() {
        this.$refs['inboundOrderRawDetailForm'].validate((valid) => {
          if (valid) {
            ...
            delete inboundOrderRawDetail.materialCode
            delete inboundOrderRawDetail.materialName
            updateInboundOrderRawDetail(inboundOrderRawDetail).then(() => {
              getInboundOrderRawItems({ inboundOrderRawDetailId: this.temp.id }).then((res) => {
                inboundOrderRawDetail.receivedQuantity = res.queryResult.total
                for (const v of this.list) {
                  if (v.id === inboundOrderRawDetail.id) {
                    const index = this.list.indexOf(v)
                    inboundOrderRawDetail.materialCode = this.temp.materialCode
                    inboundOrderRawDetail.materialName = this.temp.materialName
                    this.list.splice(index, 1, inboundOrderRawDetail)
                    break
                  }
                }
                this.dialogFormVisible = false
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除该不良代码, 是否继续?', '提示', {
          ...
          deleteInboundOrderRawDetail(row.id).then(() => {
            this.$notify({
              title: '成功',
              message: '删除成功',
              type: 'success',
